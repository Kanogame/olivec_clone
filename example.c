...
bool triangle_example() {
    ...
    olivec_fill_triangle(pixels, WIDTH, HEIGHT, x1, y3, x2, y2, x3, y1, 0xFF00FFFF);
...
bool alpha_example() {
    ...
    olivec_fill_rect(pixels, WIDTH, HEIGHT, 0, 0, WIDTH*3/4, HEIGHT*3/4, 0xFF00FF00);
    olivec_fill_rect(pixels, WIDTH, HEIGHT, WIDTH/4, HEIGHT/4, WIDTH, HEIGHT, 0xFFFFFF00);

    const char *file_path = "alpha.ppm";
    Errno err = olivec_save_to_ppm_file(pixels, WIDTH, HEIGHT, file_path);
    if (err) {
        fprintf(stderr, "ERROR: could not save file %s: %s\n", file_path, strerror(errno));
        return false;
